...
for path in ${BUILD_SUMMARY_DIRS}; do
	...
	unset catchup_config_noone_attended
	...
	# image found AND catchup_config_noimage or catchup_config_noone_attended set
	if [ -n "${catchup_image_extension}" ] && {
			[ -n "${catchup_config_noimage:-}" ] ||
			[ -n "${catchup_config_noone_attended:-}" ];
		}
	then
		echo "error: found image for catchup ${catchup_number}, but 'catchup_config_noimage' or 'catchup_config_noone_attended' set";
		echo "please unset 'catchup_config_noimage' and 'catchup_config_noone_attended' in ${catchup_config_file}";
		exit 1;
	# else, image not found
	elif [ -z "${catchup_image_extension}" ]; then
		# if catchup_config_noimage and catchup_config_noone_attended is not set
		if [ -z "${catchup_config_noimage:-}" ] && [ -z "${catchup_config_noone_attended:-}" ]; then
	...
	# Add template to combined summary page
	{
		...
		if [ -n "${catchup_noone_attended}" ]; then
			echo ":catchup_noone_attended:        ${catchup_noone_attended}";
		else
			echo ":!catchup_noone_attended:";
		fi
