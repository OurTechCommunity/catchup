[#{catchup_display_number}]
=== link:/summary/{catchup_display_number}[OTC CatchUp #{catchup_display_number}]
{catchup_authors_line}

include::sessions/{catchup_number}/content.adoc[]

include::sessions/{catchup_number}/attendees.adoc[]

==== Meet Screenshot

// with image
ifndef::catchup_no_image,catchup_noone_attended[]
image::{catchup_number}.{catchup_image_extension}[alt="Meet #{catchup_display_number} screenshot", 600, 600]
endif::[]

// no image
ifdef::catchup_no_image[]
Oops, we forgot to take a screenshot this time

video::oops.mp4[alt="Oops, we forgot to take a screenshot for CatchUp #{catchup_display_number}", opts="autoplay,muted,loop"]
endif::[]

// no one attended
ifdef::catchup_noone_attended[]
image::noone-joined.jpg[alt="When no one shows up to your google meet...\nJust paint happy little trees", 600, 600]
endif::[]


'''
